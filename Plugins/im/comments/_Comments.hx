...
class Comments {
	...
	public function read () :Void {
		...
		read_req.readFile ( URL + IMConfig.RND() );
	}
...
/*		: Xml.parse ( xml.toString() );*/
	...
	public function write (comment:Comment) :Void {
		...
		var url2 = URL.split( IMConfig.API_DIR ).pop();
			url2 = IMConfig.PHOTOS_PATH + url2.split( IMConfig.PHOTOS_PATH ).pop();
		
		// Send the data to server
		write_req = new RCHttp ( IMConfig.API_DIR );
