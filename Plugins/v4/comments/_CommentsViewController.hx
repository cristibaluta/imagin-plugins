...
class CommentsViewController extends RCView {
	...
	function config () {	
		_background_color = Preferences.hexColorForKey ("color_background_timeline");
		_text_color = Preferences.hexColorForKey ("color_text");
		_color_highlighted = Preferences.hexColorForKey ("color_highlighted");
	}
	function init () {
	...
	function write () :Void {
		...
		if (confirmation == null) {
			var format = (Preferences.stringForKey("font_type_menu", string) == "default")
			? RCFontManager.getFont("default")
			: RCFontManager.getFont("regular", {size: Preferences.stringForKey("font_size_menu", integer)});
